...
public class DefaultURLResolverService implements URLResolverService {
    ...
    @Override
    public String resolveUrl(String urlContext, boolean addProxyContextPath, boolean addWebContextRoot,
                             String tenantDomain, Map<String, Object> properties) throws URLResolverException {

        if (urlContext.startsWith("http")) {

            try {
                URL url = new URL(urlContext);
                StringBuilder urlBuilder = new StringBuilder(url.getProtocol())
                        .append(url.getHost())
                        .append(url.getPort());
                appendContextToUri(url.getPath(), addProxyContextPath, addWebContextRoot, urlBuilder);
                return urlBuilder.toString();

            } catch (MalformedURLException e) {
                throw new URLResolverException("Error while parsing the URL: " + urlContext, e);
            }
        }
    ...
    private static void appendContextToUri(String endpoint, boolean addProxyContextPath, boolean addWebContextRoot,
        ...
        // TODO: Read the value from a config
        boolean addTenantQualifier = true;
